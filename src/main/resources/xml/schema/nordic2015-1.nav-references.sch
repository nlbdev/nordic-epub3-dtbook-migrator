...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2" xmlns:c="http://www.w3.org/ns/xproc-step">
    ...
    <pattern id="nav_references_1">
        <rule context="c:result/c:result">
            ...
            <assert test="count(//html:nav[tokenize(@epub:type,'\s+')='toc']//html:a[replace(@href,'^.*/','') = ($sectioning-ref, $heading-ref)]) = 1">[nordic_nav_references_1] All headings in the
                content documents must be referenced exactly once in the navigation document. In the content document "<value-of select="replace(@xml:base,'.*/','')"/>", the heading "<value-of
                    select="text()"/>"<value-of select="if (@data-heading-id) then concat(' with id=&quot;', @data-heading-id,'&quot;') else ''"/> inside the "<value-of
                    select="@data-sectioning-element"/>" element<value-of select="if (@data-heading-id) then concat(' with id=&quot;', @data-sectioning-id,'&quot;') else ''"/> is either not
                referenced, or referenced multiple times, from the navigation document.</assert>
        </rule>
    </pattern>

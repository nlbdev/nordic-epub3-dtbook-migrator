...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <sch:title>DTBook 2005 Schematron tests for NORDIC 2010-1 rules - adapted for HTML5</sch:title>
    ...
    -->
        <sch:rule context="html:p">
            <sch:report test="html:ul | html:ol">[nordic07] Lists are not allowed inside paragraphs.</sch:report>
            <sch:report test="html:dl">[nordic07] Definition lists are not allowed inside paragraphs.</sch:report>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern id="epub_NORDIC_9">
        <sch:rule context="html:*">
            <sch:report
                ...
                >[nordic09] Element may not be empty</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 10: Metadata for dc:language, dc:date and dc:publisher must exist in the single-HTML representation -->
    <sch:pattern id="epub_NORDIC_10">
        <sch:rule context="html:head[following-sibling::html:body/html:header]">
            <!-- dc:language -->
            <sch:assert test="count(html:meta[@name='dc:language'])>=1">[nordic10] Meta dc:language must occur at least once</sch:assert>
            <!-- dc:date -->
            <sch:assert test="count(html:meta[@name='dc:date'])=1">[nordic10] Meta dc:date=YYYY-MM-DD must occur once</sch:assert>
            <sch:report test="html:meta[@name='dc:date' and translate(@content, '0123456789', '0000000000')!='0000-00-00']">[nordic10] Meta dc:date must have format YYYY-MM-DD</sch:report>
            <!-- dc:publisher -->
            <sch:assert test="count(html:meta[@name='dc:publisher'])=1">[nordic10] Meta dc:publisher must occur once</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 11: Root element must have @xml:lang -->
    <sch:pattern id="epub_NORDIC_11">
        <sch:rule context="html:html">
            <sch:assert test="@xml:lang">[nordic11] Root element must have an xml:lang attribute</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 12: Frontmatter starts with doctitle and docauthor -->
    <sch:pattern id="epub_NORDIC_12">
        <sch:rule context="html:body/html:header">
            <sch:assert test="html:*[1][self::html:h1[tokenize(@epub:type,' ')='fulltitle']]">[nordic12] Single-HTML document must begin with a fulltitle headline</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 13: All documents must have frontmatter and bodymatter -->
    <sch:pattern id="epub_NORDIC_13_a">
        <sch:rule context="html:body[html:header]">
            <sch:assert test="((html:section|html:article)/tokenize(@epub:type,'\s+')=('cover','frontmatter')) = true()">[nordic13a] A Single-HTML document must have at least one frontmatter or cover
                section</sch:assert>
            <sch:assert test="((html:section|html:article)/tokenize(@epub:type,'\s+')='bodymatter') = true()">[nordic13a] A Single-HTML document must have at least one bodymatter section</sch:assert>
            <sch:assert test="not(tokenize(@epub:type,'\s+')=('cover','frontmatter','bodymatter','backmatter'))">[nordic13a] The Single-HTML document must not have cover, frontmatter, bodymatter or
                backmatter as epub:type on its body element</sch:assert>
        </sch:rule>
    </sch:pattern>

    <sch:pattern id="epub_NORDIC_13_b">
        <sch:rule
            ...
            <sch:assert test="not((tokenize(@epub:type,'\s+')=('cover','frontmatter','bodymatter','backmatter')) = true())">[nordic13b] The Single-HTML document must not have cover, frontmatter,
    ...
    <sch:pattern id="epub_NORDIC_13_c">
        <sch:rule context="html:body[not(html:header|html:nav)]">
            <sch:assert test="tokenize(@epub:type,'\s+')=('cover','frontmatter','bodymatter','backmatter')">[nordic13c] The document must have either cover, frontmatter, bodymatter or backmatter as
                epub:type on its body element</sch:assert>
        </sch:rule>
    </sch:pattern>

    <sch:pattern id="epub_NORDIC_13_d">
        <sch:rule context="html:*[self::html:section or self::html:article][ancestor::html:body[not(html:header|html:nav)]]">
            <sch:assert test="not((tokenize(@epub:type,'\s+')=('cover','frontmatter','bodymatter','backmatter')) = true())">[nordic13d] The document must not have cover, frontmatter, bodymatter or
    ...
    <sch:pattern id="epub_NORDIC_14">
        <sch:rule context="html:*[self::html:body[not(html:header)] or self::html:section or self::html:article][not(tokenize(@epub:type,'\s+')='cover')][html:section|html:article]">
            <sch:assert test="html:h1 | html:h2 | html:h3 | html:h4 | html:h5 | html:h6">[nordic14] sectioning element with no headline (h1-h6) when sub-section is present (is only allowed for sectioning
                element with epub:type="cover")</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 20: No imggroup in inline context -->
    <sch:pattern id="epub_NORDIC_20">
        <sch:rule context="html:figure">
            <sch:report
                ...
                >[nordic20] Image groups are not allowed in inline context</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 21: No nested tables -->
    <sch:pattern id="epub_NORDIC_21">
        <sch:rule context="html:table">
            <sch:report test="ancestor::html:table">[nordic21] Nested tables are not allowed</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 23: Increasing pagebreak values for page-normal -->
    <sch:pattern id="epub_NORDIC_23">
        <sch:rule
            ...
            <sch:assert test="number(current()/@title) > number($preceding/@title)">[nordic23] pagebreak values must increase for pagebreaks with class="page-normal" (see pagebreak with
    ...
    <sch:pattern id="epub_NORDIC_24">
        <sch:rule context="html:*[tokenize(@epub:type,' ')='pagebreak'][tokenize(@class,' ')='page-front']">
            <sch:assert test="count(//html:*[tokenize(@epub:type,'\s+')='pagebreak' and tokenize(@class,'\s+')='page-front' and @title=current()/@title])=1">[nordic24] pagebreak values must be unique for
    ...
    <sch:pattern id="epub_NORDIC_26">
        <sch:rule context="html:a[tokenize(@epub:type,' ')='note']">
            <!--  		<sch:assert test="count(key('noterefs', @id))>=1">[nordic26] Each note must have at least one noteref</sch:assert>-->
            <sch:assert test="count(//html:a[tokenize(@epub:type,' ')='noteref'][translate(@idref, '#', '')=current()/@id])>=1">[nordic26] Each note must have at least one noteref</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 27: Each annotation must have an annoref -->
    <sch:pattern id="epub_NORDIC_27">
        <sch:rule context="html:aside[tokenize(@epub:type,' ')='annotation']">
            <!--  		<sch:assert test="count(key('annorefs', @id))>=1">[nordic27] Each annotation must have at least one annoref</sch:assert>-->
            <sch:assert test="count(//html:a[tokenize(@epub:type,' ')='annoref'][translate(@idref, '#', '')=current()/@id])>=1">[nordic27] Each annotation must have at least one annoref</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 29: No block elements in inline context -->
    <sch:pattern id="epub_NORDIC_29a">
        <sch:rule
            ...
            <sch:report
                ...
                >[nordic29] Block element <sch:name/> used in inline context</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 29: No block elements in inline context - continued -->
    <sch:pattern id="epub_NORDIC_29b">
        <sch:rule
            context="html:*[self::html:address    or self::html:aside[tokenize(@epub:type,' ')='annotation'] or self::html:*[tokenize(@epub:type,' ')='z3998:author' and not(parent::html:header[parent::html:body])]   or
  	                          self::html:blockquote or self::html:p[tokenize(@epub:type,' ')='bridgehead'] or self::html:caption  or
  	                          self::html:*[tokenize(@class,' ')='dateline']   or self::html:div        or self::html:dl       or
            ...
            <sch:report
                ...
                >[nordic29] Block element <sch:name/> as sibling to inline element</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 29: No block elements in inline context - continued -->
    <sch:pattern id="epub_NORDIC_29c">
        <sch:rule
            ...
            <sch:report
                test="descendant::html:*[self::html:address    or self::html:aside[tokenize(@epub:type,' ')='annotation'] or self::html:*[tokenize(@epub:type,' ')='z3998:author' and not(parent::html:header[parent::html:body])]   or
                                           ...
                                           self::html:*[tokenize(@epub:type,' ')='z3998:poem']       or self::html:*[(self::figure or self::aside) and tokenize(@epub:type,'s')='sidebar']    or self::html:table    or
                                           self::html:*[matches(local-name(),'^h\d$') and tokenize(@class,' ')='title']]"
                >[nordic29] Prodnote in inline context used as block element</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 40: No page numbering gaps for pagebreak w/page-normal -->
    <sch:pattern id="epub_NORDIC_40">
        <sch:rule
            ...
            <sch:report test="number($preceding-pagebreak/@title) != number(@title)-1">[nordic40a] No gaps may occur in page numbering (see pagebreak with title="<sch:value-of select="@title"/>" and
                compare with pagebreak with title="<sch:value-of select="$preceding-pagebreak/@title"/>")</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 50: image alt attribute -->
    <sch:pattern id="epub_NORDIC_50_a">
        <sch:rule context="html:img[parent::html:figure/tokenize(@class,'\s+')='image']">
            <sch:report test="string(@alt)!='image'">[nordic50a] an image inside a figure with class='image' must have attribute alt="image"</sch:report>
        </sch:rule>
    </sch:pattern>

    <sch:pattern id="epub_NORDIC_50_b">
        <sch:rule context="html:img[not(parent::html:figure/tokenize(@class,'\s+')='image')]">
            <sch:report test="string(@alt)!=''">[nordic50b] an image which is not inside a figure with class='image' is irrelevant or redundant with regards to the understanding of the book, so the alt
                attribute must be present but empty</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 51 & 52: -->
    <sch:pattern id="epub_NORDIC_5152">
        <sch:rule context="html:img">
            <sch:assert test="contains(@src,'.jpg') and substring-after(@src,'.jpg')=''">[nordic52] Images must have the .jpg file extension.</sch:assert>
            <sch:report test="contains(@src,'.jpg') and string-length(@src)=4">[nordic52] Images must have a base name, not just an extension.</sch:report>
            <sch:report test="not(matches(@src,'^images/[^/]+$'))">[nordic51] Images must be in the "images" folder (relative to the HTML file).</sch:report>
            <sch:assert test="string-length(translate(substring(@src,1,string-length(@src)-4),'-_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789/',''))=0">[nordic52] Image file name
    ...
    <sch:pattern id="epub_NORDIC_59">
        <sch:rule context="html:dl/html:*[tokenize(@epub:type,' ')='pagebreak']">
            <sch:assert test="not(parent::*/html:dd or parent::*/html:dt)">[nordic59] pagebreak in definition list must not occur as siblings to dd or dt</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 63: Only note references within the same document -->
    <sch:pattern id="epub_NORDIC_63">
        <sch:rule context="html:a[tokenize(@epub:type,' ')='noteref']">
            <sch:assert test="not(contains(@idref, '#')) or starts-with(@idref,'#')">[nordic63] Only note references within the same document are allowed</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 64: Only annotation references within the same document -->
    <sch:pattern id="epub_NORDIC_64">
        <sch:rule context="html:a[tokenize(@epub:type,' ')='annoref']">
            <sch:assert test="not(contains(@idref, '#')) or starts-with(@idref,'#')">[nordic64] Only annotation references within the same document are allowed</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 67: fulltitle only allowed in frontmatter -->
    <sch:pattern id="epub_NORDIC_67">
        <sch:rule context="html:h1[tokenize(@epub:type,' ')='fulltitle'][not(ancestor::html:header[parent::html:body])]">
            <sch:assert test="ancestor::html:*[tokenize(@epub:type,' ')='frontmatter']">[nordic67] fulltitle is only allowed in frontmatter</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 68: No smilref attributes -->
    <sch:pattern id="epub_NORDIC_68">
        <sch:rule context="html:*">
            <sch:report test="@smilref">[nordic68] smilref attributes in a plain DTBook file is not allowed</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 93: Some elements may not start of end with whitespace -->
    <sch:pattern id="epub_NORDIC_93">
        <sch:rule context="html:*[self::html:h1 or self::html:h2 or self::html:h3 or self::html:h4 or self::html:h5 or self::html:h6]">
            <sch:report test="normalize-space(substring(.,1,1))=''">[nordic93] element <sch:name/> may not have leading whitespace</sch:report>
            <sch:report test="normalize-space(substring(.,string-length(.),1))='' and not(html:* and normalize-space(html:*[last()]/following-sibling::text())='')">[nordic93] element <sch:name/> may not
                have trailing whitespace</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 96: no nested prodnotes or image groups -->
    <sch:pattern id="epub_NORDIC_96_a">
        <sch:rule context="html:*[tokenize(@epub:type,' ')='z3998:production']">
            <sch:report test="ancestor::html:*[tokenize(@epub:type,' ')='z3998:production']">[nordic96a] nested production notes are not allowed</sch:report>
        </sch:rule>
    </sch:pattern>

    <sch:pattern id="epub_NORDIC_96_b">
        <sch:rule context="html:figure[tokenize(@class,'\s+')='image-series']">
            <sch:report test="ancestor::html:figure[tokenize(@class,'\s+')='image-series']">[nordic96b] nested image-series are not allowed</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 101: All imggroup elements must have a img element -->
    <sch:pattern id="epub_NORDIC_101">
        <sch:rule context="html:figure[tokenize(@class,'\s+')='image']">
            <sch:assert test="html:img">[nordic101] There must be an img element in every figure with class="image"</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 103: No img without imggroup -->
    <sch:pattern id="epub_NORDIC_103">
        <sch:rule context="html:img[string-length(@alt)!=0]">
            <sch:assert test="parent::html:figure">[nordic103] There must be a figure element wrapping every img with a non-empty alt text</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 104: Headings may not be empty elements -->
    <sch:pattern id="epub_NORDIC_104">
        <sch:rule context="html:*[self::html:h1 or self::html:h2 or self::html:h3 or self::html:h4 or self::html:h5 or self::html:h6]">
            <sch:report test="normalize-space(.)=''">[nordic104] Heading <sch:name/> may not be empty</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 105: Pagebreaks must have a page-* class and must not contain anything -->
    <sch:pattern id="epub_NORDIC_105">
        <sch:rule context="html:*[tokenize(@epub:type,' ')='pagebreak']">
            <sch:assert test="tokenize(@class,' ')=('page-front','page-normal','page-special')">[nordic105] Page breaks must have either a 'page-front', a 'page-normal' or a 'page-special'
                class.</sch:assert>
            <sch:assert test="count(*|comment())=0 and string-length(string-join(text(),''))=0">[nordic105] Pagebreaks must not contain anything</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 110: pagebreak in headings -->
    <sch:pattern id="epub_NORDIC_110">
        <sch:rule context="html:*[tokenize(@epub:type,' ')='pagebreak']">
            <sch:report test="ancestor::*[self::html:h1 or self::html:h2 or self::html:h3 or self::html:h4 or self::html:h5 or self::html:h6]">[nordic110] pagebreak elements are not allowed in
                headings</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 116: Don't allow arabic numbers in pagebreak w/page-front -->
    <sch:pattern id="epub_NORDIC_116">
        <sch:rule context="html:*[tokenize(@epub:type,' ')='pagebreak']">
            <sch:report test="tokenize(@class,' ')='page-front' and translate(.,'0123456789','xxxxxxxxxx')!=.">[nordic116] Arabic numbers when @class="page-front" are not allowed</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 120:  Allow only pagebreak before hx in section/article -->
    <sch:pattern id="epub_NORDIC_120">
        <sch:rule context="html:*[self::html:h1 or self::html:h2 or self::html:h3 or self::html:h4 or self::html:h5 or self::html:h6]">
            <sch:assert test="not(preceding-sibling::html:*) or preceding-sibling::html:*[tokenize(@epub:type,' ')='pagebreak']">[nordic120] Only pagebreaks are allowed before the heading
                <sch:name/>.</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 121:  pagebreaks in tables must not occur between table rows -->
    <sch:pattern id="epub_NORDIC_121">
        <sch:rule context="html:*[tokenize(@epub:type,' ')='pagebreak'][ancestor::html:table]">
            <sch:assert test="not(../html:tr)">[nordic121] Page numbers in tables must not be placed between table rows.</sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern id="epub_NORDIC_123">
        <sch:rule context="html:*[(self::html:section or self::html:article) and count(//html:body/html:header) and count(ancestor::html:section | ancestor::html:article) = 0]">
            <sch:report test="tokenize(@epub:type,'\s+')='cover' and tokenize(@epub:type,'\s+')=('frontmatter','bodymatter','backmatter')">[nordic123] Cover (Jacket copy) is a document partition and can
    ...
    <sch:pattern id="epub_NORDIC_124">
        <sch:rule context="html:body[html:nav]">
            <sch:assert test="count(html:nav[tokenize(@epub:type,'\s+')='page-list']) &gt; 0">[nordic124] The publication must contain pagebreaks, and they must be referenced from a &lt;nav
                epub:type="page-list"&gt; in the navigation document. There is no such element in the navigation document.</sch:assert>
            <sch:assert test="count(html:nav[tokenize(@epub:type,'\s+')='page-list']//html:a) &gt; 0">[nordic124] The publication must contain pagebreaks, and they must be referenced from the &lt;nav
    ...
    <sch:pattern id="epub_NORDIC_125">
        <sch:rule context="html:img">
            <sch:assert test="string-length(@src)>=5">[nordic125] Invalid image filename.</sch:assert>
            <sch:assert test="substring(@src,string-length(@src) - 3, 4)='.jpg'">[nordic125] Images must be in JPG (*.jpg) format.</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 126: pagebreak must not occur directly after hx unless the hx is preceded by a pagebreak -->
    <sch:pattern id="epub_NORDIC_126">
        <sch:rule context="html:*[tokenize(@epub:type,' ')='pagebreak']">
            <sch:report
                ...
                >[nordic126] pagebreak must not occur directly after hx unless the hx is preceded by a pagebreak</sch:report>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern id="epub_NORDIC_128_a">
        <sch:rule context="html:html[//html:body/html:header]">
            <sch:assert test="namespace-uri-for-prefix('nordic',.)='http://www.mtm.se/epub/'">[nordic128a] xmlns:nordic="http://www.mtm.se/epub/" must be defined on the root html element.</sch:assert>
        </sch:rule>
    </sch:pattern>

    <sch:pattern id="epub_NORDIC_128_b">
        <sch:rule context="html:head[//html:body/html:header]">
            <sch:assert test="count(html:meta[@name='nordic:guidelines'])=1">[nordic128b] nordic:guidelines metadata must occur once.</sch:assert>
        </sch:rule>
    </sch:pattern>

    <sch:pattern id="epub_NORDIC_128_c">
        <sch:rule context="html:meta[//html:body/html:header][@name='nordic:guidelines']">
            <sch:assert test="@content='2015-1'">[nordic128c] nordic:guidelines metadata value must be 2015-1.</sch:assert>
        </sch:rule>
    </sch:pattern>

    <sch:pattern id="epub_NORDIC_128_d">
        <sch:rule context="html:head[//html:body/html:header]">
            <sch:assert test="count(html:meta[@name='nordic:supplier'])=1">[nordic128d] nordic:supplier metadata must occur once.</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 129: Class attribute of p -->
    <!--<sch:pattern id="epub_NORDIC_129">
    <sch:rule context="html:p[@class]">
      <sch:assert
        ...
        >[nordic129] Class attribute of &lt;p&gt; must be one (or more) of 'precedingemptyline', 'precedingseparator', 'indented' or 'asciimath'</sch:assert>
    </sch:rule>
  </sch:pattern>-->

    <!-- Rule 130 (44): dc:language must equal root element xml:lang -->
    <sch:pattern id="epub_NORDIC_130">
        <sch:rule context="html:meta[@name='dc:language']">
            <sch:assert test="@content=/html:html/@xml:lang">[nordic130] dc:language metadata must equal the root element xml:lang</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 131 (35): Allowed values in xml:lang -->
    <sch:pattern id="epub_NORDIC_131">
        <sch:rule context="*[@xml:lang]">
            <sch:assert test="matches(@xml:lang,'^[a-z][a-z](-[A-Z][A-Z]+)?$')">[nordic131] xml:lang must match '^[a-z][a-z](-[A-Z][A-Z]+)?$'</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 133: Disallowed elements -->
    <sch:pattern id="epub_NORDIC_133">
        <sch:rule
            ...
            <sch:assert test="false()">[nordic133] Element <sch:name/> is disallowed</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 134: Disallowed attributes -->
    <!--<sch:pattern id="epub_NORDIC_134">
        <sch:rule context="html:a[tokenize(@epub:type,' ')='note']">
            <sch:report test="@class">[nordic134] Attribute 'class' is not allowed on the <sch:name/> element</sch:report>
        </sch:rule>
        <sch:rule context="html:a[tokenize(@epub:type,' ')='noteref']">
            <sch:report test="@class">[nordic134] Attribute 'class' is not allowed on the <sch:name/> element</sch:report>
        </sch:rule>
        <sch:rule context="html:meta">
            <sch:report test="@scheme">[nordic134] Attribute 'scheme' is not allowed on the <sch:name/> element</sch:report>
            <sch:report test="@http-equiv">[nordic134] Attribute 'http-equiv' is not allowed on the <sch:name/> element</sch:report>
        </sch:rule>
    </sch:pattern>-->

    <!-- Rule 135: Poem contents -->
    <sch:pattern id="epub_NORDIC_135_a">
    ...
    <!-- Rule 136: List types -->
    <!--<sch:pattern id="epub_NORDIC_136">
    <sch:rule context="html:*[self::html:ul or self::html:ol]">
      <sch:assert test="@type='pl'">[nordic136] Lists must be of type 'pl' (with any bullets or numbers in the text node)</sch:assert>
    </sch:rule>
  </sch:pattern>-->

    <!-- Rule 140: Jacket copy must contain at least one part of the cover, at most one of each @class value and no other elements -->
    <sch:pattern id="epub_NORDIC_140">
        <sch:rule context="html:body[tokenize(@epub:type,'\s+')='cover'] | html:section[tokenize(@epub:type,'\s+')='cover']">
            <sch:assert test="count(*[not(matches(local-name(),'h\d'))])=count(html:section[tokenize(@class,'\s+')=('frontcover','rearcover','leftflap','rightflap')])">[nordic140] Only sections with one
                of the classes 'frontcover', 'rearcover', 'leftflap' or 'rightflap' is allowed in cover</sch:assert>
            <sch:assert test="count(html:section[tokenize(@class,'\s+')=('frontcover','rearcover','leftflap','rightflap')])>=1">[nordic140] There must be at least one section with one of the classes
                'frontcover', 'rearcover', 'leftflap' or 'rightflap' in cover.</sch:assert>
            <sch:report test="count(html:section[tokenize(@class,'\s+')='frontcover'])>1">[nordic140] Too many sections with class="frontcover" in cover</sch:report>
            <sch:report test="count(html:section[tokenize(@class,'\s+')='rearcover'])>1">[nordic140] Too many sections with class="rearcover" in cover</sch:report>
            <sch:report test="count(html:section[tokenize(@class,'\s+')='leftflap'])>1">[nordic140] Too many sections with class="leftflap" in cover</sch:report>
            <sch:report test="count(html:section[tokenize(@class,'\s+')='rightflap'])>1">[nordic140] Too many sections with class="rightflap" in cover</sch:report>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 142: Only tokenize(@class,' ')='page-special' in level1/@class='nonstandardpagination' -->
    <sch:pattern id="epub_NORDIC_142">
        <sch:rule context="html:*[tokenize(@epub:type,' ')='pagebreak'][ancestor::html:section[@class='nonstandardpagination']]">
            <sch:assert test="tokenize(@class,' ')='page-special'">[nordic142] The class page-special must be used in section/@class='nonstandardpagination'</sch:assert>
        </sch:rule>
    </sch:pattern>

    <!-- Rule 143: Don't allow pagebreak as siblings to list items or inside the first list item -->
    <sch:pattern id="epub_NORDIC_143_a">
        <sch:rule context="html:*[tokenize(@epub:type,' ')='pagebreak'][parent::html:*[self::html:ul or self::html:ol]]">
            <sch:report test="../html:li">[nordic143a] pagebreak is not allowed as sibling to list items</sch:report>
        </sch:rule>
    </sch:pattern>

    <sch:pattern id="epub_NORDIC_143_b">
        <sch:rule context="html:*[tokenize(@epub:type,' ')='pagebreak'][parent::html:li]">
            <sch:assert test="../preceding-sibling::html:li or preceding-sibling::* or preceding-sibling::text()[normalize-space()]">[nordic143b] pagebreak is not allowed at the beginning of the first

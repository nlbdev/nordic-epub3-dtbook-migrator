...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2" xml:lang="en">
    ...
    <pattern id="epub_nordic_96_a">
        ...
        <rule context="html:figure[tokenize(@class, '\s+') = 'image-series']">
            <let name="context" value="concat('(&lt;', name(), string-join(for $a in (@*) return concat(' ', $a/name(), '=&quot;', $a, '&quot;'), ''), '&gt;)')"/>
            <report test="ancestor::html:figure[tokenize(@class, '\s+') = 'image-series']">[nordic96a] Nested image series figures are not allowed. Image figures must use the class "image", while image series figures must use the class "image-series". <value-of select="$context"/></report>
        </rule>
    </pattern>
    ...
    <pattern id="epub_nordic_96_b">
        ...
        <rule context="html:figure[html:img[tokenize(@role, '\s+') = 'doc-cover']]">
            <let name="context" value="concat('(&lt;', name(), string-join(for $a in (@*) return concat(' ', $a/name(), '=&quot;', $a, '&quot;'), ''), '&gt;)')"/>
            <report test="html:aside[tokenize(@class, '\s+') = 'fig-desc']">[nordic96b] Aside elements with class="fig-desc" are not allowed in figure elements containing an img with role="doc-cover". <value-of select="$context"/></report>
        </rule>
    </pattern>

    ...
    <pattern id="opf_nordic_3">
        ...
        <rule context="opf:metadata">
            ...
            <assert test="count(dc:language[not(@refines)]) = 1 and matches(dc:language[not(@refines)]/text(), '^[a-z]{2,3}(-[A-Za-z0-9]+)*$')">[opf3c] the language code ("<value-of
                    select="dc:language[not(@refines)]/text()"/>") must be either a "two-letter lower case" code or a "two-letter lower case and groups of hyphen followed by numbers or letters" (i.e. zh-Hanz-UTF8) code.</assert>

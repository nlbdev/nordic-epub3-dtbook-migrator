...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:template match="@*|node()">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:if test="self::*
                              and (parent::*/text() or $preserve-empty-whitespace='false')
                              and normalize-space(string-join(parent::*/text(),''))=''">
                    <xsl:text>&#xa;</xsl:text>
                ...
                <xsl:copy exclude-result-prefixes="#all">
                    <xsl:apply-templates select="@*|node()"/>
                    <xsl:if test="not(self::*[matches(local-name(),'^h[d\d]$')])
                                  and self::*
                                  and *
                                  and (text() or $preserve-empty-whitespace='false')
    ...
    <xsl:template match="text()[not(ancestor::*[matches(local-name(),'^h[d\d]$')])
                                and not(.='') and normalize-space(.)='']">
        <xsl:text> </xsl:text>
    </xsl:template>

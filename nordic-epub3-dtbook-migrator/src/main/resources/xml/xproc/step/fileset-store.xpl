...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc" xmlns:px="http://www.daisy.org/ns/pipeline/xproc" xmlns:cx="http://xmlcalabash.com/ns/extensions" xmlns:err="http://www.w3.org/ns/xproc-error"
    xmlns:d="http://www.daisy.org/ns/pipeline/data" xmlns:c="http://www.w3.org/ns/xproc-step" type="pxi:fileset-store" name="main" exclude-inline-prefixes="#all" version="1.0"
        ...
        <p:choose>
            ...
            <p:when test="//d:file[resolve-uri(@href,base-uri(.))=$target]">
                ...
                <p:choose>
                    ...
                    <p:when test="/c:data or (starts-with($media-type,'text/') and not(starts-with($media-type,'text/xml')))">
                        <p:store method="text">
                            <p:with-option name="href" select="$target"/>
                    ...
                    <p:when test="$method='xhtml'">
                        <p:store encoding="utf-8" method="xhtml" include-content-type="false">
                            <p:with-option name="href" select="$target"/>
                            <p:with-option name="byte-order-mark" select="$byte-order-mark"/>

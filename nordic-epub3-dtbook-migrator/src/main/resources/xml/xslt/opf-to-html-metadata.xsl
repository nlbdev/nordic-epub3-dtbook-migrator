...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:opf="http://www.idpf.org/2007/opf" xmlns="http://www.w3.org/1999/xhtml"
    ...
    <xsl:template match="opf:metadata">
        <head xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/">
            ...
            <title>
                <xsl:copy-of select="dc:title[1]/(@scheme|@http-equiv|@id|@xml:lang|@dir|namespace::*[not(.=('http://www.w3.org/1999/xhtml','http://www.idpf.org/2007/opf'))]|node())"/>
            </title>
            <xsl:for-each select="*[not(self::opf:*) and not(self::dc:title)]">
                <meta name="{name()}" content="{normalize-space(.)}">
                    <xsl:copy-of select="@scheme|@http-equiv|@id|@xml:lang|@dir|namespace::*[not(.=('http://www.w3.org/1999/xhtml','http://www.idpf.org/2007/opf'))]"/>
                </meta>
            </xsl:for-each>
